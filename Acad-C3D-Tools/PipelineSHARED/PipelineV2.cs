...
namespace IntersectUtilities.PipelineNetworkSystem
{
    ...
    public abstract class PipelineV2Base : IPipelineV2
    {
        ...
        public EntityCollection PipelineEntities
        {
            get => _pipelineEntities;
        }
        public EntityCollection PipelineWelds
        {
            get => _pipelineWelds;
        }
        ...
        public PipelineV2Base(IEnumerable<Entity> source)
        {
            ...
            catch (Exception)
            {
                if (_pipelineEntities == null)
                    ...
                    foreach (var entity in _pipelineEntities)
                    {
        ...
        public Point3d GetLocationForMaxDN()
        {
            ...
            if (_pipelineSizes.Length == 1)
                return this.StartPoint;

            if (
                _pipelineSizes.Sizes.First().System == PipeSystemEnum.Stål
                && _pipelineSizes.Sizes.Last().System == PipeSystemEnum.Stål
            )
            {
                if (_pipelineSizes.Sizes.First().DN > _pipelineSizes.Sizes.Last().DN)
                    return this.StartPoint;
                else
                    return this.EndPoint;
            }
            else if (_pipelineSizes.Sizes.First().System == PipeSystemEnum.Stål)
                return this.StartPoint;
            else if (_pipelineSizes.Sizes.Last().System == PipeSystemEnum.Stål)
                return this.EndPoint;
            else
            {
                if (_pipelineSizes.Sizes.First().DN > _pipelineSizes.Sizes.Last().DN)
    ...
    public class PipelineV2Na : PipelineV2Base
    {
        ...
        public PipelineV2Na(IEnumerable<Entity> source)
            : base(source)
        {
            ...
            var query = db.GetFjvEntities(tx, true, false)
                .Where(x => _pipelineEntities.All(y => x.Handle != y.Handle));

            Dictionary<Handle, Ent> externalEntities = query.ToDictionary(
                x => x.Handle,
                x => new Ent(x, _psh)
            );
            Dictionary<Handle, Ent> localEntities = _pipelineEntities.ToDictionary(
                x => x.Handle,
                x => new Ent(x, _psh)
            );
            ...
            var connectedExternalEntities = localEntities
                .SelectMany(ent =>
                    ent.Value.Cons.Where(con => externalEntities.ContainsKey(con.ConHandle))
            ...
            Ent TraverseForValidNode(Ent connectedExternalEntity)
            {
                ...
                while (stack.Count > 0)
                {
                    ...
                    string alignment = _psh.Pipeline.ReadPropertyString(
                        currentNode.Entity,
                        _psh.PipelineDef.BelongsToAlignment
                    );
                    ...
                    {
                        if (
                            externalEntities.TryGetValue(con.ConHandle, out var neighbor)
            ...
            var temp = externalRootNode
                .Cons.Where(x => localEntities.ContainsKey(x.ConHandle))
                .FirstOrDefault();
            if (temp == null)
                throw new Exception(
                    $"Could not find connecting node for pipeline {Name}!\n"
        ...
        public override double GetStationAtPoint(Point3d pt) =>
            topology.GetDistAtPoint(topology.GetClosestPointTo(pt, false));

        public override Point3d GetClosestPointTo(Point3d pt, bool extend = false) =>
            topology.GetClosestPointTo(pt, extend);
        ...
        /// <summary>
        /// The parent's path as a polyline. An alignment's is a new database
        /// resident polyline the caller must erase; an NA's is its topology.
        /// </summary>
        private static Polyline TopologyOf(IPipelineV2 parent) => parent switch
        {
            PipelineV2Alignment pal => pal
                .al.GetPolyline()
                .Go<Polyline>(pal.al.Database.TransactionManager.TopTransaction)
                ?? throw new Exception($"Alignment {pal.Name} gave no polyline!"),
            PipelineV2Na pna => pna.topology,
            _ => throw new Exception($"Unknown pipeline type {parent.GetType()}!"),
        };
            ...
            out Point3d location, out Entity? partAtJoint)
        {
            ...
            if (ent1 != null)
                ...
                location = parentPlRef.GetClosestPointTo(thisEnd, false);
            else
                return false;
        ...
        private class Ent
        {
            ...
            public Ent(Entity entity, PropertySetHelper psh)
            {
                ...
                string conString = psh.Graph.ReadPropertyString(
                    entity,
                    psh.GraphDef.ConnectedEntities
                );
                if (conString.IsNoE())
            ...
            public Point3d DetermineConnectionPoint(Ent other)
            {
                ...
                foreach (var pt in EndPoints)
                {
                    foreach (var otherPt in other.EndPoints)
                        if (pt.DistanceHorizontalTo(otherPt) < 0.001)
                            return pt;
                }
                //If we get here, need to check connection to polyline
                //else there's a problem
                if (Entity is Polyline pl1 && other.Entity is Polyline pl2)
                    return Point3d.Origin;
                else if (Entity is Polyline pl)
                {
                    var query = other.EndPoints.Where(x =>
                        pl.GetClosestPointTo(x, false).DistanceHorizontalTo(x) < 0.001
                    );
                    if (query.Any())
                        return query.First();
                }
                ...
                {
                    var query = EndPoints.Where(x =>
                        pl3.GetClosestPointTo(x, false).DistanceHorizontalTo(x) < 0.001
                    );
                    if (query.Any())
                        return query.First();
                }
