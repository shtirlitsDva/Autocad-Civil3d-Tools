...
namespace PipeScheduleV2Tests
{
    internal static class PipeScheduleV2Report
    {
        public static string WriteHtmlReport(List<Ps2Result> results, string reportsDir)
        {
            ...
            sb.AppendLine($"<p>DWG: {System.Web.HttpUtility.HtmlEncode(db.Filename)}<br/>UTC: {System.DateTime.UtcNow:u}</p>");
            ...
            foreach (var r in results)
            {
                ...
                sb.AppendLine("<tr>");
                sb.AppendLine($"<td>{System.Web.HttpUtility.HtmlEncode(r.Name)}</td>");
                sb.AppendLine($"<td class=\"{cls}\">{r.Status}</td>");
                sb.AppendLine($"<td>{r.Duration.TotalMilliseconds:F0}</td>");
                sb.AppendLine($"<td>{System.Web.HttpUtility.HtmlEncode(r.Message)}</td>");
                sb.AppendLine($"<td><pre>{System.Web.HttpUtility.HtmlEncode(r.StackTrace)}</pre></td>");
                sb.AppendLine("</tr>");
            }
