...
namespace DimensioneringV2.UI.ApplyDim
{
    internal class ResetDimManager : IDisposable
    {
        ...
        private void OnHoverTimerTick(object sender, EventArgs e)
        {
            ...
            //Null check here makes the path not update
            //So if we have drawn a path and hover outside features
            //The path should revert to start feature only
            //But with null check this doesn't happen
            //So the path stays highlighted until we hover another feature
            //Recommended to remove null check so path resets also outside features
