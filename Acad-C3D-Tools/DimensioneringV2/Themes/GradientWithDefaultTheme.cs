...
namespace DimensioneringV2.Themes
{
    class GradientWithDefaultTheme : StyleBase, IThemeStyle
    {
        ...
        public IStyle? GetStyle(IFeature row)
        {
            ...
            //Added line to (hopefully) handle zero values
            if (attr < this.Min) return StyleProvider.BasicStyle;
        ...
        private static VectorStyle ToInterpolatedVectorStyle(VectorStyle min, VectorStyle max, double value, GradientWithDefaultTheme instance)
        {
        ...
        //private static ImageStyle ToInterpolatedSymbolStyle(ImageStyle min, ImageStyle max, double value, GradientWithDefaultTheme instance)
        //{
        ...
        private LabelStyle ToInterpolatedLabelStyle(LabelStyle min, LabelStyle max, double value, GradientWithDefaultTheme instance)
        {
        ...
        {
            if (Math.Abs(fraction - 1) < Mapsui.Utilities.Constants.Epsilon)
                return maxColor;
            if (Math.Abs(fraction - 0) < Mapsui.Utilities.Constants.Epsilon)
                return minColor;
