...
namespace IntersectUtilities
{
    public partial class Intersect
    {
        ...
        /// "Afløbsledning" onto their nearest 3D main line. On open it gathers every such polyline and classifies it as
        /// 3D (real elevations) or 2D. A check distance groups the touching 3D lines into colour-coded networks and assigns
        /// each 2D line to the nearest network within that distance; the 3D/2D split and the main↔child network can be
        /// previewed transiently. "Anvend tilslutninger" extends each connected 2D line's nearest end along its tangent to
        /// intersect the main in XY, lifts that point to the main's real elevation as a pivot, and rebuilds the line as
        /// a new 3D polyline sloping upward away from the pivot at the given per-mille. The original 2D line is erased and
        /// the rebuilt line keeps the source layer and properties. 2D lines with no main within the distance, or whose
        /// tangent never meets the main, are left untouched and reported. The 3D join is in-memory only — original 3D
