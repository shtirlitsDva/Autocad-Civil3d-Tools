                ...
                switch (e)
                {
                    case TPipe p:
                        ...
                        if (isTwin)
                        {
                            ...
                            var odMm = IntersectUtilities.PipeScheduleV2.PipeScheduleV2.GetPipeOd(p.System, p.Dn);
                            var gapMm = IntersectUtilities.PipeScheduleV2.PipeScheduleV2.GetPipeDistanceForTwin(p.System, p.Dn, p.Type);
                            zLower = -Math.Max(0.0, (odMm + gapMm)) / 1000.0;
                        }

                        void EmitSegment(NtrGraph g0, Pt2 a0, Pt2 b0, double s0, double s1)
                        {
                            var soil = Covered(p.CushionSpans, s0, s1) ? new NTRExport.SoilModel.SoilProfile("Soil_C80", 0.08) : NTRExport.SoilModel.SoilProfile.Default;
                            // upper (return)
